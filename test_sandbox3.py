...
async def main():
    ...
    # Inject via placeholder comment
    template = Path("/app/static/pm_sandbox.html").read_text(encoding="utf-8")
    data_script = "<script>window.__GRAPH_DATA__ = %s;</script>" % graph_json
    html = template.replace("<!--GRAPH_DATA_PLACEHOLDER-->", data_script)
    
    preview_path = Path("/app/static/_pm_preview_test3.html")
    ...
    async with async_playwright() as p:
        ...
        await page.goto("file://" + str(preview_path), wait_until="networkidle", timeout=15000)
        ...
        errs = await page.evaluate("JSON.stringify(window.__JS_ERRORS__ || [])")
        print("Data loaded: %s, Layout done: %s" % (has_data, is_done))
        print("JS errors: %s" % errs)
        
        png_bytes = await page.screenshot(type="png", full_page=True)
        await browser.close()
    
    out_path = Path("/app/static/meiyijia_preview3.png")
