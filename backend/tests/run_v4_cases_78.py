"""美宜佳 v4 测试 Case 7 & 8 — 删部门留人 / 删部门删人

Regenerates v4_07 and v4_08 SVGs with proper tree layout (v31) + v4_local_layout.
...
# ── SVG style constants (matching v4_00~v4_06 format) ──
BG_FILL = "#fafafa"
DEPT_FILL = "#e9f5e9"
DEPT_STROKE = "#b8d4b8"
DEPT_TEXT = "#2d5a2d"
...
def load_baseline() -> MergeContext:
    """Load raw BI data → PowerNode list → v31 global layout → MergeContext."""
    with open(DATA_FILE, "r", encoding="utf-8") as f:
        raw = json.load(f)

    # Convert to PowerNode (parent_dept_id fix already in _node_from_bi_dict)
    nodes = [_node_from_bi_dict(nd) for nd in raw["nodes"]]
    edges = raw.get("edges", [])[:]

    # Sort depts first, then users — _v31_global_layout single-pass
    # grouping requires dept nodes already in dept_nodes dict when
    # a user's parent_dept_id is checked.
    nodes.sort(key=lambda n: 0 if n.node_type == "dept" else 1)

    # ── v3.1 global layout: tree + dept sizing + orphan placement ──
    _v31_global_layout(nodes, edges)

    # Re-mark anomalies after layout (some may now be locked)
    _mark_geometry_anomalies(nodes)

    # Build merge context with all lookups
...
def render_svg(ctx: MergeContext, title: str, new_ids: set, filepath: Path):
    ...
    # Edges (center-to-center, with opacity)
    ...
    # Department nodes (drawn first so user nodes are on top)
    for n in nodes:
        if n.node_type != "dept":
            continue
    ...
    for n in nodes:
        ...
        if is_locked:
            ...
            name_color = LOCKED_TEXT  # v4_00 uses #c62828 for all user names

        L(f'<rect x="{n.x}" y="{n.y}" width="{n.w}" height="{n.h}" rx="8" '
        ...
        if n.position:
            L(f'<text x="{cx}" y="{n.y+42}" text-anchor="middle" font-size="10" fill="{POSITION_COLOR}">{n.position}</text>')
...
def run_case(label: str, delta: dict, out_name: str) -> tuple[MergeContext, list]:
    ...
    ctx = load_baseline()
    initial_count = len(ctx.all_nodes)

    # Track new nodes (for coloring) before apply
    old_ids = {n.id for n in ctx.all_nodes}

    # Apply delta
    ctx = _apply_delta(ctx, delta)
    after_apply = len(ctx.all_nodes)

    # Identify new/modified nodes for coloring
    new_ids = set()
    for n in ctx.all_nodes:
        if n.id not in old_ids:
            new_ids.add(n.id)

    # Compute forced move set
...
print(f"  SUMMARY")
print(f"{'='*60}")
print(f"  Case 7: {'PASS' if not coll7 else 'COLLISIONS DETECTED'}")
print(f"  Case 8: {'PASS' if not coll8 else 'COLLISIONS DETECTED'}")
